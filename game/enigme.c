...
if(f!=NULL)
{
while(fscanf(f,"%s %s %s %s %d\n",question,rep1,rep2,rep3,&reponse)!=EOF)
{
nbligne++;
}
fclose(f);
...
if(f!=NULL)
{
while(fscanf(f,"%s %s %s %s %d\n",e->question,e->rep1,e->rep2,e->rep3,&e->reponse)!=EOF)
{
check++;
if(check==alea){break;}
}
...
e->posreponse3.x=400;
e->posreponse3.y=150;
fontquest=TTF_OpenFont ("",30);
font=TTF_OpenFont("Trajan Pro.ttf",20);
SDL_Color black={0,0,0};
SDL_Color white={255,255,255};
e->surfacequestion=TTF_RenderText_Blended(fontquest,e->question,black);
e->surfacereponse1=TTF_RenderText_Blended(font,e->reponse1,white);
e->surfacereponse2=TTF_RenderText_Blended(font,e->reponse2,white);
e->surfacereponse3=TTF_RenderText_Blended(font,e->reponse3,white);
}
//Score
